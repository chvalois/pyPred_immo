...
def generate_models_A(suffix):
    ...
    model_experiment = mlflow.set_experiment(f"model_A_{suffix}_{datetime_now}")
    ...
    # Modèle global A : estimation du prix de base
    logging.info(f'Generating model | Tous biens')
    rf_all, results_all = random_forest_model(X_train, X_test, y_train, y_test, suffix, "A", "tous_biens", datetime_now, params_a)
    reduc_dim_lasso(df, X_train, X_test, y_train, y_test, column_list, "global", suffix)

    # Modèle A1 - Appartements Province
    logging.info(f'Generating model | Modèle 1 | Appartements hors Paris')
    rf_1, results_1 = random_forest_model(X_train_1, X_test_1, y_train_1, y_test_1, suffix, "A", "appartements_province", datetime_now, params_a)
    reduc_dim_lasso(df_1, X_train_1, X_test_1, y_train_1, y_test_1, column_list, "appart_province", suffix)

    # Modèle A2 - Appartements Paris
    logging.info(f'Generating model | Modèle 2 | Appartements Paris')
    rf_2, results_2 = random_forest_model(X_train_2, X_test_2, y_train_2, y_test_2, suffix, "A", "appartements_paris", datetime_now, params_a)
    reduc_dim_lasso(df_2, X_train_2, X_test_2, y_train_2, y_test_2, column_list, "appart_paris", suffix)

    # Modèle A3 - Maisons Province
    logging.info(f'Generating model | Modèle 3 | Maisons Province')
    rf_3, results_3 = random_forest_model(X_train_3, X_test_3, y_train_3, y_test_3, suffix, "A", "maisons_province", datetime_now, params_a)
    ...
    joblib.dump(rf_all, f'models/{suffix}_model_A_all.pkl', compress = 3)
    joblib.dump(rf_1, f'models/{suffix}_model_A_1_appart_province.pkl', compress = 3)
    joblib.dump(rf_2, f'models/{suffix}_model_A_2_appart_paris.pkl', compress = 3)
    joblib.dump(rf_3, f'models/{suffix}_model_A_3_maison_province.pkl', compress = 3)

    # Export de la liste des colonnes de X_train
    feature_list = list(X_train.columns)
    print(X_train.dtypes)
    df_feat = pd.DataFrame({'feature_name': feature_list})
    df_feat.to_csv(f'models/{suffix}_features_list_for_model_A.csv')